    ...
    <div class="shelf-footer" v-show="isEditMode">
      <div class="shelf-footer-tab-wrapper" v-for="item in tabs" :key="item.index" @click="onTabClick(item)" >
        <div class="shelf-footer-tab" :class="{'is-selected':isSelected}">
          ...
          <div class="tab-text" :class="{'romve-text':item.index === 4}">{{label(item)}}</div>
        </div>
      </div>
    </div>
</template>
...
export default {
  ...
  methods:{
    ...
    onComplete(){
      this.hidePopup()//隐藏弹出Popup
      this.setIsEditMode(false)//取消编辑状态
      this.setIsAllMode(true)//更新全选状态
      saveBookShelf(this.shelfList)//将我们所有的修改状态保存到书架当中
    },
    setPrivate(){//是否开启私密模式
      let isPrivate
    ...
    async setDownload(){//是否开启缓存图书
      ...
      if(this.isDownload){
        ...
        await this.downloadSelectedBook()//调用数据缓存方法
