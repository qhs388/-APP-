  ...
  <transition name="fade">
    <div class="shelf-title" v-show="shelfTitleVisible" :class="{'hide-shadow':ifHideShadow}">
      ...
      <div class="shelf-title-btn-wrapper shelf-title-left" v-if="showClear">
        <span class="shelf-title-btn-text" @click="clearCache">{{isEditMode ?( isAllMode ? $t('shelf.SelectAll') : $t('shelf.NoSelectAll') ):$t('shelf.clearCache') }}</span>
      </div>
      ...
      if(this.isEditMode){
        if(this.isAllMode){
          this.shelfList.forEach(item=>{
           if(item.type===1){
            item.selected = true
            this.shelfSelected.pushWithoutDuplicate(item)
           }
          })
          this.setIsAllMode(!this.isAllMode)
        }else{
          this.setShelfSelected([])
          this.shelfList.forEach(item => {
              item.selected = false
